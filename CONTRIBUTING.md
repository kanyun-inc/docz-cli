...
感谢你贡献 docz-cli！本文说明如何开发、提 PR、自动发版。
...
```bash
git checkout -b feature-<简述>     # 新功能（也支持 beta 预发布，见下文）
git checkout -b fix-<简述>         # Bug 修复
```
...
按提示选：
- 版本类型：
  - `patch` — Bug 修复、小调整（0.7.1 → 0.7.2）
  - `minor` — 新命令、新 MCP 工具、新功能（0.7.1 → 0.8.0）
  - `major` — Breaking change（0.7.1 → 1.0.0）
...
docz-cli 用 [changesets](https://github.com/changesets/changesets) + GitHub Actions **全自动**发版。你不用改 `package.json` 的 version、不用打 tag、不用 `npm publish`、不用输 OTP。
...
# CI 自动发 docz-cli@beta
# 用户可用 npm i docz-cli@beta 体验
...
- 提 Issue：https://github.com/kanyun-inc/docz-cli/issues
